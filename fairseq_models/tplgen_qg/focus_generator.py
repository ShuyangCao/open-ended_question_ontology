...
class FocusSequenceGenerator(nn.Module):
    def __init__(
        self,
        models,
        tgt_dict,
...
class EnsembleModel(nn.Module):
    ...
    def forward_encoder(self, net_input: Dict[str, Tensor]):
        ...
        if not self.has_encoder():
            return None
        if hasattr(self.models[0], 'forward_encoder'):
        ...
        for i, model in enumerate(self.models):
            ...
            if self.has_incremental_states():
                decoder_out = model.decoder.forward(
                    ...
                    focus_encoder_out=encoder_out[1],
                    incremental_state=incremental_states[i],
                )
            else:
                decoder_out = model.decoder.forward(tokens, encoder_out=encoder_out[0],
                    focus_encoder_out=encoder_out[1])
        ...
        for i, model in enumerate(self.models):
            ...
            new_outs.append(
                (model.encoder.reorder_encoder_out(encoder_outs[i][0], new_order),
                 model.encoder.reorder_encoder_out(encoder_outs[i][1], new_order))
            )
        return new_outs
